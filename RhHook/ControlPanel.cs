  ...
  public class ControlPanel : Form
  {
    ...
    public delegate int DLoadGlobalClass();
        ...
        private void timer3_Tick(object sender, EventArgs e)
        {
            ...
            if(seconds == 9900)
            {
                Notice.SendNotice("Santa Claus - Boss Assault will respawn at the center of Coliseum in 15 minutes and do not forget to take the quest from Obivos, located at Gathering Hall.", 1);
            }
            if (seconds == 10200)
            {
                Notice.SendNotice("Santa Claus - Boss Assault will respawn at the center of Coliseum in 10 minutes and do not forget to take the quest from Obivos, located at Gathering Hall.", 1);
            }
            if (seconds == 10500)
            {
                Notice.SendNotice("Santa Claus - Boss Assault will respawn at the center of Coliseum in 5 minutes and do not forget to take the quest from Obivos, located at Gathering Hall.", 1);
            }

            if (seconds == 10800) //3hours = 10800
            {
                try
                {
                    ...
                    for (int i = 0; i < dataTable.Rows.Count; i++)
                    {
                        ...
                        float x = float.Parse(dataRow["x_pos"].ToString());
                        float y = float.Parse(dataRow["y_pos"].ToString());
                        float z = float.Parse(dataRow["z_pos"].ToString());
            ...
            {
                //RANDOM ANNOUNCEMENT EVERY 1 HOUR AFTER KILLING BOSS ASSAULT
        ...
        private void timer4_Tick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (label2.Text == "19:00:00")
                {
                    SqlCommand log = new SqlCommand("UPDATE RohanUser.dbo.TUser set cookie = 'NoLogin'", selectConnection);
                    SqlCommand log2 = new SqlCommand("UPDATE RohanUser.dbo.TUser set reward = 'Unclaim'", selectConnection);
                    selectConnection.Open();
                    log.ExecuteNonQuery();
                    log2.ExecuteNonQuery();
        ...
        {
            Notice.SendNotice(this.textBox1.Text, 2);
        }
    }
}
