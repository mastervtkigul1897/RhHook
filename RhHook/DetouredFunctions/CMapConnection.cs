...
namespace RhHook.DetouredFunctions
{
    internal class CMapConnection
    {
        ...
        public static readonly CMapConnection.DGenericpacket CMonster_DeadProcess = new CMapConnection.DGenericpacket(CMapConnection.DetouredCMonster_DeadProcess);
        ...
        public static readonly RhHook.Util.FunctionInfo[] FunctionInfo = new RhHook.Util.FunctionInfo[4]
        {
      new RhHook.Util.FunctionInfo((IntPtr) 5658448, "CMonster::DeadProcess", (Delegate) CMapConnection.CMonster_DeadProcess, (Delegate) CMapConnection._Genericreq, typeof (CMapConnection.DGenericreq)),
      ...
      new RhHook.Util.FunctionInfo((IntPtr) 4673424, "CMapConnection::OnLeaveGuildNew", (Delegate) CMapConnection.OnLeaveGuildNew, (Delegate) CMapConnection._GenericT, typeof (CMapConnection.DGenericT))
        ...
        private static int DetouredCMonster_DeadProcess(IntPtr @this, IntPtr prcreq)
        {
            try
            {
                string monsterid = ((CEntityBase)Marshal.PtrToStructure(@this, typeof(CEntityBase))).m_ID.m_nType.ToString();
                ...
                /** GET PARTY ID */
                SqlDataAdapter sqlDataAdapter2 = new SqlDataAdapter("SELECT * FROM RohanGame.dbo.TCharacter where id = " + prcreq + "", selectConnection);
                ...
                for (int i = 0; i < dataTable2.Rows.Count; i++)
                {
                    ...
                    partyid = dataRow["party_id"].ToString();
                    charactername = dataRow["name"].ToString();

                }
                if(monsterid == "302154")
                {
                    if(int.Parse(partyid) != 0)
                    {
                        /** GET WINNERS AND INSERT REWARD */
                        Console.ForegroundColor = ConsoleColor.DarkCyan;
                        Console.WriteLine("Player: {0} has killed the Boss Assault but no party detected..", charactername);
                        Console.ResetColor();
                        SqlDataAdapter sqlDataAdapter3 = new SqlDataAdapter("SELECT * FROM RohanGame.dbo.TCharacter where party_id = " + partyid + "", selectConnection);
                        DataTable dataTable3 = new DataTable();
                        sqlDataAdapter3.Fill(dataTable3);
                        for (int i = 0; i < dataTable3.Rows.Count; i++)
                        {
                            StringBuilder stringBuilder = new StringBuilder();
                            DataRow dataRow = dataTable3.Rows[i];
                            string getwinnersname = dataRow["name"].ToString();
                            string charid = dataRow["id"].ToString();
                            SqlCommand log = new SqlCommand("INSERT INTO RohanEvents.dbo.TBossAssaultWin (name,date) VALUES ('" + getwinnersname + "',GETDATE())", selectConnection);
                            SqlCommand sqlCommand1 = new SqlCommand("INSERT INTO [RohanGame].[dbo].[TEventItem] SELECT 3507886,0x00,1,0,0,0,0,0,id,GETDATE() FROM RohanGame.dbo.TCharacter where id = " + charid + "", selectConnection);
                            selectConnection.Open();
                            log.ExecuteNonQuery();
                            sqlCommand1.ExecuteNonQuery();
                            selectConnection.Close();
                            Console.ForegroundColor = ConsoleColor.Green;
                            Console.WriteLine("Player Win: {0} ", getwinnersname);
                            Console.ResetColor();

                        }
                        Notice.SendNotice(charactername + "'s Party has been rewarded a quest item for killing Santa Claus - Boss Assault. Congratulations!" +
                            System.Environment.NewLine + System.Environment.NewLine +
                            "Santa Claus - Boss Assault will respawn again in 3 hours after his death!", 1);
                        ControlPanel.seconds = 0;
                    }
                    else
                    {
                        Notice.SendNotice(charactername + " Killed the Santa Claus - Boss Assault But No Party Detected. Better Luck Next Time!" +
                            System.Environment.NewLine + System.Environment.NewLine +
                            "Santa Claus - Boss Assault will respawn again in 3 hours after his death!", 1);
                ...
                return (int)((Dictionary<string, Hook>)Hooks.Manager)["CMonster::DeadProcess"].CallOriginal(new object[2]
                {
          (object) @this,
          (object) prcreq
                });
